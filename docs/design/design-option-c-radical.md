...
Two existing build systems survive deletion essentially intact. Weapon class composition grants +8% damage at 2-of-a-class and +18% at 3-of-a-class, and the same calculation is used by server damage and client preview (`packages/shared/src/weapons.ts:1166-1190`; `packages/server/src/rooms/GameRoom.ts:3230-3290`; `packages/client/src/ui/level-up-model.ts:404-427`). Pre-made dual weapons keep both render pieces and their authored alternating/both-hand combo inside one definition and one slot. Under this proposal, only the words “grades” and their multiplier path change; class composition, authored cadence, rarity, affixes, three equipped slots plus bag, scrip, archiving, and bank-or-lose extraction are valuable weapon identity and should remain.
...
3. **Levels 12–20 — combine:** the second and third equipped weapons create class-set and coverage decisions; pre-made duals remain one-slot authored choices, while augments at 15 and 20 make interactions rather than merely raise totals.
...
| Attribute requirements | Delete the damage penalty. Grip, rarity, world tier, bank/carry risk, three slots, and class composition already gate access. A requirement without allocatable attributes is only a hidden tax. |
...
- Damage, cooldown, reload/refill, beam heat, crit, class-set multiplication, rarity/affix multiplication, authored-dual combo output, boss time-to-kill, XP pacing, and enemy scaling all need new reference builds at levels 1/5/10/20/30. Requirements currently suppress many weapons early; removing them changes both average and worst-case power even before any tune is selected.
...
- The `chooseAttribute` message, pending counters, timeout loop, freeze/untarget policy, client modal, prediction freeze, level-up pure model, weapon card scaling/requirement art, menu stat preview, and broad test fixtures all change (`packages/server/src/rooms/GameRoom.ts:2301-2327`, `9675-9710`; `packages/client/src/net/prediction.ts:114`; `packages/client/src/scenes/arena/card-art.ts:732-758`; `packages/client/src/scenes/MenuScene.ts:1566`, `1930`).
...
- Weapon ranks must follow an instance through active slots, bag, pickup/drop, reconnect, and reforge without entering permanent bank value. That touches the arsenal transaction paths even though rarity, affix, scrip, and archiving rules do not conceptually change.
- `packages/shared/src/leveling.ts`, `weapons.ts`, `combat.ts`, `gear.ts`, `state.ts`, `character-classes.ts`, and generated character/weapon data all have type or formula dependencies. Core server impact is concentrated in `progression.ts` and the very large `GameRoom.ts`; core client impact is concentrated in `level-up-model.ts`, `ArenaScene.ts`, card art, and menu previews. The associated tests are extensive and valuable—they must be rewritten around new invariants, not deleted for expediency.
...
- **Composition:** keep +8%/+18% weapon class thresholds and pre-made dual grip identities. These are simple, visible reasons to care about the loadout as a set.
