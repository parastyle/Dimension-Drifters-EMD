...
- Three equipped weapons create a class build: two matching class weapons grant +8% class damage and three grant +18% (`weapons.ts:1166-1190`). Runtime pairing is narrower than “two 1H weapons”: both must be different, genuinely 1H, in the same class, and in the same live delivery lane; beams, thrown weapons, authored duals, ground zones, and Fists are excluded (`weapons.ts:1094-1118`). These are good rules, but a player should see eligibility and the reason for ineligibility before attempting an action.
- The game already has pieces of the proposed answer. Weapon cards show grip/family/element, damage-source equations, scaling-grade chips, and color-coded requirements (`packages/client/src/scenes/arena/card-art.ts:661-775`). The metagame Armory has search plus filters for zone, class, rarity, pair/single, and sorting (`packages/client/src/scenes/MenuScene.ts:2130-2145, 2272-2302`), and its model can also filter family, delivery, provenance, and composition even though not all are exposed as toolbar controls (`packages/client/src/ui/armory/model.ts:52-77, 308-343`). The missing piece is contextual comparison: the Armory detail currently reports identity, tags, value, size, and source, but not “what happens to my build if I equip this?” (`MenuScene.ts:2297-2315`).
- Dual-wield preview is the strongest existing pattern to copy. It already states combined damage, each hand's cadence, separate-magazine behavior, fee, and the literal eligibility law (`packages/client/src/scenes/ArenaScene.ts:14685-14723`). Option A extends that standard to every ordinary equip decision.
...
There should be no universal DPS number. Damage coverage, resource cadence, area, chain falloff, range, pairing, and set changes make a single score falsely authoritative. Show arithmetic where it is comparable and behavior where it is not. The existing dual-wield preview—combined damage, each hand's cadence, separate magazines, and eligibility law—is already the right standard.
...
- **Do not remove three equipped slots, bag decisions, 2/3 class bonuses, same-class/same-delivery dual pairing, requirements, rarity, affixes, scrip, or archiving.** These systems create loadout stories. Their rules should travel with the relevant action.
