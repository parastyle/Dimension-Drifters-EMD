...
export const PET_META_STORAGE_KEY = "dd.metaAccount.v2";
...
function legacyMetaAccount(): MetaAccountV2 {
...
export function loadPetMetaAccount(): MetaAccountV2 {
  try {
    const raw = localStorage.getItem(PET_META_STORAGE_KEY);
    if (raw) return sanitizeMetaAccountV2(JSON.parse(raw));
    const migrated = legacyMetaAccount();
    savePetMetaAccount(migrated);
    return migrated;
  } catch {
    return createMetaAccountV2();
  }
}

/** Persist a complete replacement response and keep the legacy belt cache coherent during rollout. */
export function savePetMetaAccount(value: unknown): MetaAccountV2 {
  const account = sanitizeMetaAccountV2(value);
  try {
    localStorage.setItem(PET_META_STORAGE_KEY, JSON.stringify(account));
    localStorage.setItem("dd.beltScrip", String(account.scrip));
    localStorage.setItem("dd.beltUpgrades", JSON.stringify(account.upgrades));
  } catch {
...
export function selectPet(account: MetaAccountV2, petId: PetId | ""): MetaAccountV2 {
  const next = sanitizeMetaAccountV2(account);
...
export function petSelectionView(account: MetaAccountV2, id: PetId): PetSelectionView {
