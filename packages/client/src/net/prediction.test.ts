...
describe("SelfPredictor — §4 v0.107 prediction + reconciliation", () => {
  ...
  it("retains attack-channel recoil across an older movement patch until its fire tick confirms", () => {
    const server = new MockServer();
    const initial = { ...server.view(), tick: 40, attackSeq: 0, attackTick: 0 };
    const pred = new SelfPredictor(initial);
    pred.addPredictedImpulse(-180, 0, undefined, {
      attackSeq: 1,
      round: 0,
      intervalTicks: 1,
    });
    expect(pred.stats.pendingOwnerImpulses).toBe(1);

    const cmd = pred.mintCmd(1, 0, false);
    pred.tick(cmd);
    pred.reconcile({ ...initial, ackSeq: 0 });
    expect(pred.stats.pendingOwnerImpulses).toBe(1);
    ...
    expect(pred.stats.pendingOwnerImpulses).toBe(0);
    expect(pred.stats.errPx).toBe(0);
  });

  it("does not rotate legitimate recoil movement into the held WASD direction", () => {
    const pred = new SelfPredictor(new MockServer().view());
    const before = pred.renderPos(1, 0, 0);
    pred.addPredictedImpulse(-780, 0, 780);
    pred.tick(pred.mintCmd(1, 0, false));
    const candidate = pred.renderPos(1, 0, 0);
    expect(candidate.x).toBeLessThan(before.x);
    const presented = pred.constrainRenderStep(
      before.x,
      before.y,
      candidate.x,
      candidate.y,
    ...
    expect(presented).toEqual({ x: candidate.x, y: candidate.y });
    expect(pred.stats.errPx).toBe(0);
  });

  it("retains the gun-only recovery bound through a smooth frame-gap resync", () => {
    const server = new MockServer();
    const initial = { ...server.view(), tick: 40, attackSeq: 0, attackTick: 0 };
    const pred = new SelfPredictor(initial);
    pred.addPredictedImpulse(-780, 0, 780, {
    ...
    pred.forceResync();
    pred.reconcile({ ...initial, tick: 41 });
    pred.foldError(200, 0);
    expect(pred.stats.errPx).toBeLessThanOrEqual(48);
    const rendered = pred.renderPos(0, 0, 0);
    const bounded = pred.boundOwnerImpulsePresentation(server.x, server.y, rendered.x, rendered.y);
    expect(Math.abs(bounded.x - server.x)).toBeLessThanOrEqual(72);
  });

  it("does not apply gun recovery bounds merely because a non-gun attack sequence advanced", () => {
    const server = new MockServer();
    const pred = new SelfPredictor({ ...server.view(), tick: 40, attackSeq: 0, attackTick: 0 });
    pred.reconcile({ ...server.view(), tick: 41, attackSeq: 1, attackTick: 41 });
    pred.foldError(200, 0);
    expect(pred.stats.errPx).toBe(200);
  });

  it("restores the recovery bound from a recent authoritative gun attack", () => {
    const server = new MockServer();
    const pred = new SelfPredictor({ ...server.view(), tick: 40, attackSeq: 0, attackTick: 0 });
    pred.reconcile({
      ...server.view(),
