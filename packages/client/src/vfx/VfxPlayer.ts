...
export class VfxPlayer {
  ...
  static preloadAssets(scene: Phaser.Scene): void {
    ...
    for (const [id, vfx] of Object.entries(WEAPON_VFX)) {
      ...
      if (vfx.generatedImage?.kind === "fan-tornado") {
        for (const frame of vfx.generatedImage.frames)
          if (!scene.textures.exists(frame.textureKey))
            scene.load.image(frame.textureKey, frame.url);
      } else if (vfx.generatedImage && !scene.textures.exists(vfx.generatedImage.textureKey)) {
