...
export default defineConfig({
  server: {
    ...
    // Authoring studios are dev-only Vite entries. Their small local servers own repo writes;
    // proxying keeps the browser same-origin and leaves production/game runtime bundles untouched.
    proxy: {
      "/api/pose-studio": {
        target: `http://127.0.0.1:${Number(process.env.POSE_STUDIO_API_PORT) || 5050}`,
      },
      "/api/walkability-painter": {
        target: `http://127.0.0.1:${Number(process.env.WALKABILITY_PAINTER_API_PORT) || 5051}`,
      },
    },
  },
