    ...
    res.on("end", async () => {
      ...
      ws.on("open", async () => {
        ...
        for (let i = 0; i < 25; i++) {
          ...
          if (
            await ev(
              `!!(window.ddGame&&${SC}.room&&${SC}.room.state.players.size>0&&${SC}.carousel&&${SC}.carousel.length)`,
            )
          )
            break;
        }
        ...
        if (!ready) {
          console.log(JSON.stringify({ error: "game/carousel not ready" }));
          ws.close();
          return;
        }
        ...
        const braced = await ev(
          `(()=>{const rig=${SC}.blobs.get(${SC}.room.sessionId);if(!rig||!rig.triggerBrace)return false;${SC}.room.send('parry');rig.triggerBrace(${SC}.time.now);return true;})()`,
        );
        await sleep(140);
