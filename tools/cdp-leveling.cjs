...
http.get("http://localhost:9222/json/list", (res) => {
  let d = ""; res.on("data", (c) => (d += c)); res.on("end", async () => {
    ...
    ws.on("open", async () => {
      ...
      let maxLevel = 1, maxPower = 1, maxProj = 0, lastMaxHp = 100, leveledShot = -1;
      for (let t = 0; t < 90; t++) {
        const th = t * 0.3;
        ...
        await ev(`(()=>{const r=${SC}.room,me=r.state.players.get(r.sessionId);let bx=1,by=0,bd=1e9;r.state.enemies.forEach(e=>{const d=Math.hypot(e.x-me.x,e.y-me.y);if(d<bd){bd=d;bx=e.x-me.x;by=e.y-me.y;}});r.send('attack',{aimX:bx,aimY:by});})()`);
        const s = await ev(`(()=>{const r=${SC}.room,me=r.state.players.get(r.sessionId);return {level:me.level,xp:Math.round(me.xp*10)/10,xpToNext:me.xpToNext,power:me.power,maxHp:me.maxHp,hp:Math.round(me.hp),proj:r.state.projectiles.size,enemies:r.state.enemies.size,alive:me.alive};})()`);
        if (s) {
          maxLevel = Math.max(maxLevel, s.level); maxPower = Math.max(maxPower, s.power); maxProj = Math.max(maxProj, s.proj); lastMaxHp = s.maxHp;
          if (s.level >= 2 && leveledShot < 0) {
            await ev(`(()=>{const s=${SC},r=s.room,me=r.state.players.get(r.sessionId);s.cameras.main.setZoom(1.3);s.cameras.main.centerOn(me.x,me.y);})()`);
            await sleep(120);
            const shot = await cmd("Page.captureScreenshot", { format: "png" });
            fs.writeFileSync("C:/Projects/Dimension Drifters v2/tools/artkit/out/_leveling_check.png", Buffer.from(shot.result.data, "base64"));
            leveledShot = t;
          }
        }
        await sleep(300);
      }
      console.log(JSON.stringify({ maxLevel, maxPower, maxHp_end: lastMaxHp, maxProjectiles: maxProj, screenshotAtTick: leveledShot }));
