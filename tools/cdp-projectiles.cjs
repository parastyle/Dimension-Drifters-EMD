    ...
    res.on("end", async () => {
      ...
      ws.on("open", async () => {
        ...
        for (let t = 0; t < 80; t++) {
          ...
          if (t % 2 === 0)
            await ev(
              `${SC}.room.send('attack',{aimX:${Math.cos(th).toFixed(3)},aimY:${Math.sin(th).toFixed(3)}})`,
            );
          const snap = await ev(
            `(()=>{const s=${SC},r=s.room;let boot=0;r.state.enemies.forEach(e=>{if(e.kind==='boothill')boot++;});const me=r.state.players.get(r.sessionId);return {proj:r.state.projectiles.size, boothill:boot, enemies:r.state.enemies.size, clientProj:s.projectiles.size, hp:Math.round(me?me.hp:0), alive:me?me.alive:false};})()`,
          );
          if (snap) {
            maxProj = Math.max(maxProj, snap.proj);
            maxBoothill = Math.max(maxBoothill, snap.boothill);
            ...
            if (snap.proj > 0 && shotAt < 0) {
              await ev(
                `(()=>{const s=${SC},r=s.room,me=r.state.players.get(r.sessionId);s.cameras.main.setZoom(1.5);s.cameras.main.centerOn(me.x,me.y);return 1;})()`,
              );
              await sleep(150);
              const shot = await cmd("Page.captureScreenshot", { format: "png" });
              fs.writeFileSync(
                "C:/Projects/Dimension Drifters v2/tools/artkit/out/_projectiles_check.png",
        ...
        console.log(
          JSON.stringify({
            maxProjectiles_server: maxProj,
            maxBoothill,
            sawProjectiles_client: sawProjClient,
            damageEvents: hits,
            deaths,
            screenshotAtTick: shotAt,
          }),
        );
        ws.close();
        process.exit(0);
