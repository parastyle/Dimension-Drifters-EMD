  ...
  .get("http://localhost:9222/json/list", (res) => {
    ...
    res.on("end", async () => {
      ...
      ws.on("open", async () => {
        ...
        await ev(
          `(()=>{const s=${SC},r=s.room,me=r.state.players.get(r.sessionId);const cam=s.cameras.main;cam.setZoom(3.2);cam.centerOn(me.x,me.y);return s.scale.gameSize.width+'x'+s.scale.gameSize.height;})()`,
        );
        await sleep(900);
        const shot = await cmd("Page.captureScreenshot", { format: "png" });
        fs.writeFileSync(
          "C:/Projects/Dimension Drifters v2/tools/artkit/out/_presize_check.png",
          Buffer.from(shot.result.data, "base64"),
        );
        console.log(
          "weapon:",
          await ev(`${SC}.room.state.players.get(${SC}.room.sessionId).weapon`),
        );
        console.log(
          "rig scale:",
          await ev(
            `(()=>{const s=${SC};const rig=s.players&&s.players.get(s.room.sessionId);return rig?rig.scale:'n/a';})()`,
          ),
        );
