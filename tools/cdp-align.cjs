const http = require("node:http"),
  WebSocket = require("C:/Projects/Dimension Drifters v2/node_modules/.pnpm/ws@8.21.0/node_modules/ws");
http
    ...
    res.on("end", async () => {
      ...
      ws.on("open", async () => {
        ...
        console.log(
          "collision:",
          await ev(
            `(()=>{const r=${SC}.room,es=[];r.state.enemies.forEach(e=>es.push(e));let stacked=0;for(let i=0;i<es.length;i++)for(let j=i+1;j<es.length;j++)if(Math.hypot(es[i].x-es[j].x,es[i].y-es[j].y)<22)stacked++;return JSON.stringify({enemies:es.length,stackedPairs_under22px:stacked});})()`,
          ),
        );
        ...
        const before = await ev(nearest);
        await ev(`${SC}.room.send('parry')`);
        await sleep(300);
        const after = await ev(nearest);
        console.log(
          "parry knockback: nearest enemy",
          before,
          "->",
          after,
          after > before ? "(pushed away ✓)" : "(no change)",
        );
        ws.close();
        process.exit(0);
      });
