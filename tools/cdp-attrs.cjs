...
http.get("http://localhost:9222/json/list", (res) => {
  let d = ""; res.on("data", (c) => (d += c)); res.on("end", async () => {
    ...
    ws.on("open", async () => {
      ...
      const out = { reachedWindow: !!atFlex, autoAtLevelUp: atFlex && { level: atFlex.level, str: atFlex.str, con: atFlex.con, others: `${atFlex.dex}/${atFlex.int}/${atFlex.luk}`, maxHp: atFlex.maxHp, flexPending: atFlex.flexPending, winObjs: atFlex.winObjs } };
      if (atFlex) {
        await sleep(250);
        const sh = await cmd("Page.captureScreenshot", { format: "png" }); fs.writeFileSync("C:/Projects/Dimension Drifters v2/tools/artkit/out/_attr_window.png", Buffer.from(sh.result.data, "base64"));
        ...
        for (let t = 0; t < 60; t++) { const th = t * 0.4; await ev(`${SC}.room.send('input',{dx:${Math.cos(th).toFixed(3)},dy:${Math.sin(th).toFixed(3)}})`); await ev(`(()=>{const r=${SC}.room,me=r.state.players.get(r.sessionId);let bx=1,by=0,bd=1e9;r.state.enemies.forEach(e=>{const dd=Math.hypot(e.x-me.x,e.y-me.y);if(dd<bd){bd=dd;bx=e.x-me.x;by=e.y-me.y;}});r.send('attack',{aimX:bx,aimY:by});})()`); const s = await ev(snap); if (s && s.flexPending > 0) { out.beforeTimeout = s; break; } await sleep(280); }
        if (out.beforeTimeout) { await sleep(6000); out.afterTimeout = await ev(snap); }
      }
