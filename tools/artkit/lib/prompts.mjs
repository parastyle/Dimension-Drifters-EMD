...
export function buildReferenceTicket(subject, style, candidateCount = 3, styleRefAttached = false, variant = null) {
  ...
  const styleRefBlock = !styleRefAttached
    ...
    : isVfx
      ...
      : "\n## ⭐ HOUSE-STYLE / BUILD REFERENCE — the attached image(s) (MATCH THEIR RENDERING + PROPORTIONS EXACTLY)\nThe attached image(s) are APPROVED, on-model Dimension Drifters characters (the LOCKED house style). Your output MUST match them on BOTH axes:\n- **Rendering:** outline weight + hand-drawn unevenness, flat-cel shadow/highlight discipline, saturation + grime level.\n- **Build & PROPORTIONS:** the rounded 'pill' construction — ONE smooth capsule/egg/cone torso (no waist, no realistic anatomy), the head-to-body ratio, the detached blob mitten-hands and chunky boots floating with clear gaps, the UPRIGHT planted stance, and the FACELESS head (a dark void with at most one or two small glowing dot-eyes, or a flat mask — never human features).\nRender as if drawn by the SAME hand, of the SAME species of little guy. The references govern STYLE + BUILD ONLY — do NOT copy their specific costume, hat/helmet/mask, silhouette, or colours; render the DIFFERENT subject described below.\n";
