...
const TOP_KEYS = new Set([
  ...
  "handlingTags", "poseLanguage", "impactMuzzle", "rapidThrust", "fireMode",
]);
...
if (!tierRows || typeof tierRows !== "object" || Array.isArray(tierRows)) {
  errors.push("weapon-tiers.json: tiers must be an object");
}
const authoredTiers = {};
for (const [id, tier] of Object.entries(tierRows ?? {})) {
  if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(id)) {
    errors.push(`weapon-tiers.json: invalid weapon id ${JSON.stringify(id)}`);
  } else if (!Number.isInteger(tier) || tier < 1 || tier > 5) {
    errors.push(`weapon-tiers.json: ${id} tier must be an integer from 1 through 5`);
  } else {
    authoredTiers[id] = tier;
  }
}
const out = {};
const comboBars = {};
for (const w of data.weapons) {
  ...
  if (!authoredTiers[w.id]) fail("missing authored tier in data/weapon-tiers.json");
...
const body =
  ...
  `export const GENERATED_WEAPONS: Record<string, WeaponDefSource> = ${JSON.stringify(out, null, 2)};\n\n` +
...
const tierBody =
  ...
  `export const GENERATED_WEAPON_TIERS = ${JSON.stringify(authoredTiers, null, 2)} as const ` +
