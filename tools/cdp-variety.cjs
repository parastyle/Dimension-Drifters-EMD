    ...
    res.on("end", async () => {
      ...
      ws.on("open", async () => {
        ...
        for (let t = 0; t < 210; t++) {
          ...
          const s = await ev(
            `(()=>{const sc=${SC},r=sc.room;let tough=0;r.state.enemies.forEach(e=>{if(e.tough)tough++;});return {zonesSrv:r.state.zones.size, zonesCli:sc.zones.size, tough, elapsed:Math.floor(r.state.elapsed)};})()`,
          );
          if (s) {
            maxZonesSrv = Math.max(maxZonesSrv, s.zonesSrv);
            maxZonesCli = Math.max(maxZonesCli, s.zonesCli);
            maxTough = Math.max(maxTough, s.tough);
            maxElapsed = Math.max(maxElapsed, s.elapsed);
            if (s.tough > 0 && !shot) {
              await ev(
                `(()=>{const sc=${SC},r=sc.room,me=r.state.players.get(r.sessionId);sc.cameras.main.setZoom(0.8);sc.cameras.main.centerOn(me.x,me.y);})()`,
              );
              await sleep(150);
              const sh = await cmd("Page.captureScreenshot", { format: "png" });
              fs.writeFileSync(
                "C:/Projects/Dimension Drifters v2/tools/artkit/out/_variety_check.png",
        ...
        console.log(
          JSON.stringify({
            maxZones_server: maxZonesSrv,
            maxZones_client: maxZonesCli,
            maxTough_concurrent: maxTough,
            maxElapsed,
            toughShot: shot,
          }),
        );
