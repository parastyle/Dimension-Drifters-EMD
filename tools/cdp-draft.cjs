    ...
    ws.on("open", async () => {
      ...
      for (let t = 0; t < 120; t++) {
        ...
        if (s && s.pending) {
          ...
          fs.writeFileSync("C:/Projects/Dimension Drifters v2/tools/artkit/out/_draft_overlay.png", Buffer.from(sh.result.data, "base64"));
          ...
          result = { reached: true, offered, picked: pick, draftCardsWhileOpen: before.draftCards,
            before: { maxHp: before.maxHp, power: before.power, speedMul: before.speedMul, regenBonus: before.regenBonus, haste: before.haste, xpMul: before.xpMul },
            after: { maxHp: after.maxHp, power: after.power, speedMul: after.speedMul, regenBonus: after.regenBonus, haste: after.haste, xpMul: after.xpMul },
            pendingClearedOrNext: after.pending, draftCardsAfter: after.draftCards };
          break;
        }
        await sleep(300);
      }
