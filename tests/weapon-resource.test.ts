...
describe("Drive formula v1", () => {
  it("covers the frozen 329-weapon catalog in deterministic id order", () => {
    ...
    expect(census).toEqual({ melee: 169, thrown: 16, gun: 118, cast: 2, beam: 21, zone: 3 });
  });
  ...
  it("pins the documented delivery distributions", () => {
    ...
    expect(bands("gun")).toEqual({ min: 1.25, median: 11, max: 35 });
