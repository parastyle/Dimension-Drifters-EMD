...
describe("B24 shared radial fallback removal", () => {
  it("records the complete 340-weapon resolver cohort after the B63/B66 catalog merge", () => {
    ...
    expect(candidates).toHaveLength(340);
    expect(byFormerLayer).toEqual({
      "blade-trail": 302,
      "twin-slash": 27,
      "thrust-streak": 11,
    });
    expect(candidates.filter((definition) => !definition.archived)).toHaveLength(322);
