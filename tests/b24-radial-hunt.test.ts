...
describe("B24 shared radial fallback removal", () => {
  it("records the complete 341-weapon resolver cohort after the B63/B66 catalog merge", () => {
    ...
    // The merged catalog plus Helix adds 18 fallback-candidate rows; authored Rimegut/Whiteout VFX stay excluded.
    expect(candidates).toHaveLength(341);
    expect(byFormerLayer).toEqual({
      "blade-trail": 303,
      "twin-slash": 27,
      "thrust-streak": 11,
    });
    expect(candidates.filter((definition) => !definition.archived)).toHaveLength(323);
  ...
  it("archives Pocket Hexicon through the canonical weapon catalogs", () => {
    ...
    // B63/B66 have added twenty-one active catalog rows so far while the archive census stays fixed.
