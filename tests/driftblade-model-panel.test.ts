...
describe("driftblade-model panel", () => {
  ...
  it("keeps every non-adopter's routing byte-identical (full-roster invariance table)", () => {
    ...
    expect(signatureGroups).toEqual({
      ...
      "punch/wing-chun-centerline-blitz": ["x2-wing-chun-wraps"],
      ...
      "chop/bardiche-hookbreak": ["x2-permafrost-bardiche"],
      "chop/quarry-frontflip-slam": ["x2-quarry-splitter-bardiche"],
    ...
    expect(defaultCounts).toEqual({
      "punch/default": 19,
      // The B63/B66 catalog merge and Rail-9 deliberately contribute ordinary arc routes.
      "arc/default": 127,
      "chop/default": 8,
      "rake/dagger": 1,
      "rake/claw": 3,
