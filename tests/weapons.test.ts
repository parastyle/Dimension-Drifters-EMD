...
describe("B20 L1 flat weapon contract", () => {
  it("removes scaling and requirement keys recursively from every shipped definition", () => {
    ...
    for (const weapon of Object.values(WEAPONS)) visit(weapon);
  });

  it("enumerates Wyrmtooth's flat blade, magma, and blast values", () => {
    ...
    expect(magma?.base).toBe(5);
  });

  it("a plain melee weapon has a single 'hit' source", () => {
    const s = weaponDamageSources({ damage: 7 } as never);
    expect(s).toHaveLength(1);
    expect(s[0]?.label).toBe("hit");
    expect(s[0]?.base).toBe(7);
  });
...
describe("§13 expansion roster (the +300, gated behind the `expansion` flag)", () => {
  ...
  it("every expansion weapon exposes only authored flat damage", () => {
    for (const id of EXPANSION_WEAPON_IDS) {
      const weapon = WEAPONS[id];
      expect(weapon?.damage).toBeGreaterThan(0);
      expect(weapon).not.toHaveProperty("scalingGrades");
      expect(weapon).not.toHaveProperty("requirements");
    }
  });
});
...
describe("beam weapons — hard panel laws", () => {
  ...
  it("keeps the 19 caster beams plus six channelled energy guns after the Micro-Lance", () => {
    ...
    expect(WEAPONS["x2-solaris-beam-lance"]?.beam).toBeDefined();
    expect(WEAPONS["x2-singularity-micro-lance"]?.beam).toBeDefined();
    expect(beamDefs).toHaveLength(25);
  ...
  it("reports beam DPS as its own flat card damage source", () => {
